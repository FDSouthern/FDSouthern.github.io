[Original file](https://github.com/jrh13/hol-light/blob/master/Library/prime.ml)
...
(*****************************************************************************)
# g `!m i n. ((SUC n) * m = (SUC n) * i) <=> (m = i)`;;
val it : goalstack = 1 subgoal (1 total)

`!m i n. SUC n * m = SUC n * i <=> m = i`

# e (REWRITE_TAC [EQ_MULT_LCANCEL]);;
val it : goalstack = 1 subgoal (1 total)

`!m i n. SUC n = 0 \/ m = i <=> m = i`

# e (REWRITE_TAC [NOT_SUC]);;
val it : goalstack = No subgoals

# let MULT_MONO_EQ = top_thm();;
...
# e (REWRITE_TAC[LT_EXISTS; ADD1; ADD_ASSOC]);;
val it : goalstack = No subgoals

# let LESS_ADD_1 = top_thm();;
val LESS_ADD_1 : thm = |- !m n. n < m ==> (?p. m = n + p + 1)
(*****************************************************************************)
...
# let LESS_ADD_SUC = top_thm();;
val LESS_ADD_SUC : thm = |- !m n. m < m + SUC n
(*****************************************************************************)
...
# let LESS_EQ_0 = top_thm();;
val LESS_EQ_0 : thm = |- !n. n <= 0 <=> n = 0
(*****************************************************************************)
...
# e (CONV_TAC (REWRITE_CONV [EXP_EQ_0]));;
val it : goalstack = 1 subgoal (1 total)

`!m n. ~(SUC n = 0 /\ ~(m = 0))`

# e (CONV_TAC (REWRITE_CONV [NOT_SUC]));;
val it : goalstack = No subgoals

# let NOT_EXP_0 = top_thm();;
val NOT_EXP_0 : thm = |- !m n. ~(SUC n EXP m = 0)
(*****************************************************************************)
...
# e (STRUCT_CASES_TAC (SPEC `x:num` num_CASES));;
val it : goalstack = 2 subgoals (2 total)

`SUC n * y = SUC n <=> SUC n = 0 \/ y = 1`
...
# e (REWRITE_TAC [GSYM (el 4 (CONJUNCTS (SPEC_ALL MULT_CLAUSES)))]);;
val it : goalstack = 1 subgoal (1 total)

`SUC n * y = SUC n <=> y = 1`

# e (GEN_REWRITE_TAC (LAND_CONV o RAND_CONV) [GSYM (el 3 (CONJUNCTS (SPEC_ALL MULT_CLAUSES)))]);;
val it : goalstack = 1 subgoal (1 total)

`SUC n * y = SUC n * 1 <=> y = 1`

# e (REWRITE_TAC [EQ_MULT_LCANCEL; NOT_SUC]);;
val it : goalstack = No subgoals

# let MULT_FIX = top_thm();;
val MULT_FIX : thm = |- !x y. x * y = x <=> x = 0 \/ y = 1
(*****************************************************************************)
...
# e (DISCH_THEN (STRIP_ASSUME_TAC o REWRITE_RULE [LE_EXISTS]));;
val it : goalstack = 1 subgoal (1 total)
...
# e (ASM_REWRITE_TAC [LEFT_ADD_DISTRIB; RIGHT_ADD_DISTRIB; GSYM ADD_ASSOC; LE_ADD]);;
val it : goalstack = No subgoals

# let LESS_EQ_MULT = top_thm();;
val LESS_EQ_MULT : thm = |- !m n p q. m <= n /\ p <= q ==> m * p <= n * q
(*****************************************************************************)
let LESS_MULT = prove(
  `!m n p q. m < n /\ p < q ==> (m * p) < (n * q)`,
...
# e (DISCH_THEN(CONJUNCTS_THEN ((CHOOSE_THEN SUBST_ALL_TAC) o MATCH_MP LESS_ADD_1)));;
val it : goalstack = 1 subgoal (1 total)
...
# e (REWRITE_TAC [GSYM ADD1; MULT_CLAUSES; ADD_CLAUSES; GSYM ADD_ASSOC]);;
val it : goalstack = 1 subgoal (1 total)

`m * p < SUC (m * p + m * p'' + m + p' * p + p' * p'' + p' + p + p'')`

# e (ONCE_REWRITE_TAC [GSYM (el 3 (CONJUNCTS ADD_CLAUSES))]);;
val it : goalstack = 1 subgoal (1 total)

`m * p < m * p + SUC (m * p'' + m + p' * p + p' * p'' + p' + p + p'')`

# e (MATCH_ACCEPT_TAC LESS_ADD_SUC);;
val it : goalstack = No subgoals

# let LESS_MULT = top_thm();;
val LESS_MULT : thm = |- !m n p q. m < n /\ p < q ==> m * p < n * q
(*****************************************************************************)
...
# e (DISJ_CASES_TAC (SPEC `a:num` (ARITH_RULE `!m. (0 = m) \/ 0 < m`)));;
val it : goalstack = 2 subgoals (2 total)
...
# e (DISCH_THEN (CHOOSE_THEN SUBST1_TAC o MATCH_MP (prove (`!m n. n < m ==> (?p. m = n + (p + 1))`, REWRITE_TAC [LT_EXISTS; ADD1; ADD_ASSOC]))));;
val it : goalstack = 1 subgoal (1 total)

  0 [`0 < a`]

`a = 0 \/ a + (a + p + 1) - a < a + a + p + 1`
...
# e (REWRITE_TAC [ONCE_REWRITE_RULE [ADD_SYM] ADD_SUB]);;
val it : goalstack = 1 subgoal (1 total)
...
# e (GEN_REWRITE_TAC LAND_CONV [GSYM (CONJUNCT1 ADD_CLAUSES)]);;
val it : goalstack = 1 subgoal (1 total)
...
# e (REPEAT (MATCH_MP_TAC (ARITH_RULE `!m n p. m < n ==> (m + p) < (n + p)`)));;
val it : goalstack = 1 subgoal (1 total)
...
# let DIFF_LEMMA = top_thm();;
val DIFF_LEMMA : thm = |- !a b. a < b ==> a = 0 \/ a + b - a < a + b
(*****************************************************************************)
...
# e (DISCH_THEN(MP_TAC o AP_TERM `EVEN`));;
val it : goalstack = 1 subgoal (1 total)

`(EVEN (2 * m) <=> EVEN (SUC (2 * n))) ==> F`

# e (REWRITE_TAC[EVEN; EVEN_MULT; ARITH]);;
val it : goalstack = No subgoals

# let NOT_EVEN_EQ_ODD = top_thm();;
val NOT_EVEN_EQ_ODD : thm = |- !m n. ~(2 * m = SUC (2 * n))
(*****************************************************************************)
